...
from mm_mnemonic.commands.show import Params
from mm_mnemonic.types import Coin


class TestShowDirect:
    """Test show command with direct mnemonic mode."""

    def test_direct_basic(self, runner: CliRunner, mnemonic: str) -> None:
        """Test basic direct mode with known mnemonic."""
        result = runner.invoke(app, ["show", "--mnemonic", mnemonic, "--limit", "1", "--allow-internet-risk"])
    ...
    def test_direct_with_passphrase(self, runner: CliRunner, mnemonic: str, passphrase: str) -> None:
        ...
        result = runner.invoke(
            app, ["show", "--mnemonic", mnemonic, "--passphrase", passphrase, "--limit", "1", "--allow-internet-risk"]
        )
    ...
    def test_direct_deterministic(self, runner: CliRunner) -> None:
        ...
        result1 = runner.invoke(app, ["show", "--mnemonic", mnemonic, "--limit", "2", "--allow-internet-risk"])
        result2 = runner.invoke(app, ["show", "--mnemonic", mnemonic, "--limit", "2", "--allow-internet-risk"])
    ...
    def test_direct_different_coins(self, runner: CliRunner) -> None:
        ...
        for coin in ["BTC", "ETH", "SOL", "TRX"]:
            result = runner.invoke(app, ["show", "--mnemonic", mnemonic, "--coin", coin, "--limit", "1", "--allow-internet-risk"])
            assert result.exit_code == 0
            assert f"Coin: {coin}" in result.stdout


class TestShowInteractive:
    """Test show command with interactive mode."""

    @patch("mm_mnemonic.commands.show.typer.prompt")
    def test_interactive_basic(self, mock_prompt: Mock, runner: CliRunner) -> None:
        ...
        result = runner.invoke(app, ["show", "--limit", "1", "--allow-internet-risk"])
...
class TestShowFileOutput:
    """Test show command file output functionality."""
    ...
    def test_save_to_file_basic(self, runner: CliRunner, tmp_path: Path) -> None:
        ...
        result = runner.invoke(
            app, ["show", "--mnemonic", mnemonic, "--output-dir", str(tmp_path), "--limit", "2", "--allow-internet-risk"]
        )
    ...
    def test_file_output_hides_sensitive_info(self, runner: CliRunner, tmp_path: Path) -> None:
        ...
        result = runner.invoke(
            app, ["show", "--mnemonic", mnemonic, "--output-dir", str(tmp_path), "--limit", "1", "--allow-internet-risk"]
        )
...
class TestShowValidation:
    ...
    def test_invalid_mnemonic(self, runner: CliRunner) -> None:
        ...
        result = runner.invoke(app, ["show", "--mnemonic", "invalid mnemonic", "--allow-internet-risk"])
    ...
    def test_invalid_coin_type(self, runner: CliRunner) -> None:
        ...
        result = runner.invoke(app, ["show", "--mnemonic", mnemonic, "--coin", "INVALID"])
...
class TestShowEdgeCases:
    """Test edge cases for show command."""
    ...
    def test_empty_output_directory_error(self, runner: CliRunner, tmp_path: Path) -> None:
        ...
        result = runner.invoke(app, ["show", "--mnemonic", mnemonic, "--output-dir", str(test_dir), "--allow-internet-risk"])

        assert result.exit_code == 1
        assert "is not empty" in result.stdout

    def test_custom_derivation_path(self, runner: CliRunner) -> None:
        """Test show with custom derivation path."""
        mnemonic = "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about"
        custom_path = "m/44'/3'/0'/0/{i}"

        result = runner.invoke(
            app, ["show", "--mnemonic", mnemonic, "--derivation-path", custom_path, "--limit", "1", "--allow-internet-risk"]
        )
